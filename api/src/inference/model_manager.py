...
class ModelManager:
    """Manages AhaTTS V1 model loading and inference."""
    ...
    def __init__(self, config: ModelConfig | None = None):
        ...
        self._backend: KokoroV1 | None = None  # Explicitly type as KokoroV1
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(f"Initializing AhaTTS V1 on {self._device}")
            self._backend = KokoroV1()

        except Exception as e:
            raise RuntimeError(f"Failed to initialize AhaTTS V1: {e}")
    ...
    async def initialize_with_warmup(self, voice_manager) -> tuple[str, str, int]:
        ...
        try:
            ...
            return self._device, "AhaTTS V1", len(voices)
        except FileNotFoundError:
            logger.error(
                """
Model files not found! Download the AhaTTS V1 model:

1. Run the download script:
   python scripts/download_model.py --output api/src/models/v1_0

2. Or download kokoro-v1_0.pth directly:
...
async def get_manager(config: ModelConfig | None = None) -> ModelManager:
