...
class CaptionedSpeechResponse(BaseModel):
    ...
    timestamps: list[WordTimestamp] | None = Field(
...
class NormalizationOptions(BaseModel):
    ...
    url_normalization: bool = Field(
        default=True,
        description="Changes urls so they can be properly pronounced by AhaTTS",
    )
    email_normalization: bool = Field(
        default=True,
        description="Changes emails so they can be properly pronouced by AhaTTS",
    )
    ...
    phone_normalization: bool = Field(
        default=True,
        description="Changes phone numbers so they can be properly pronouced by AhaTTS",
    )
...
class OpenAISpeechRequest(BaseModel):
    ...
    model: str = Field(
        default="AhaTTS",
        description="The model to use for generation. Supported models: tts-1, tts-1-hd, AhaTTS",
    )
    ...
    download_format: Literal["mp3", "opus", "aac", "flac", "wav", "pcm"] | None = (
        Field(
            default=None,
